...
export const associationController = {
    ...
    async getOne(req, res,next) {
        ...
        const association = await Association.findByPk(id, {
                            include: [{association: 'pets', attributes:["id", "name", "birthdate"], include: 'pictures'}, {association:"requests", attributes:["status", "starting_date", "ending_date", "pet_id"]}], // on affiche les animaux de l'association sélectionnée - on n'affiche que name et birthdate + les requetes de cette association
                            attributes: ["id", "name", "email", "logo", "zip_code", "address", "city", "phone_number", "description"] // on affiche les champs name et city de l'association
        });
        if (!association) {
